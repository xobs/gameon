...
static const uint8_t hid_report_descriptor[] = {
 0x05, 0x01,                  // (GLOBAL) USAGE_PAGE         0x0001 Generic Desktop Page
  ...
  0x85, 0x07,                  //   (GLOBAL) REPORT_ID          0x02 (2)
  0x19, 0x01,                  //   (LOCAL)  USAGE_MINIMUM      0xFF000001
  0x29, 0x20,                  //   (LOCAL)  USAGE_MAXIMUM      0xFF000020
  ...
  0x95, 0x08,                  //   (GLOBAL) REPORT_COUNT       0x08 (8) Number of fields
  0xB1, 0x00,                  //   (MAIN)   FEATURE            0x00000000 (8 fields x 8 bits) 0=Data 0=Array 0=Absolute 0=NoWrap 0=Linear 0=PrefState 0=NoNull 0=NonVolatile 0=Bitmap
  0xC0,                        // (MAIN)   END_COLLECTION     Application
};
...
static int get_set_hid_report_descriptor(struct GrainuumUSB *usb,
...
static int get_report(struct GrainuumUSB *usb,
                          const void *packet,
                          const void **response)
{
  static const uint8_t data[8] = {0, 0, 10, 0, 0, 3, 0x2800 / 1024, 0};
  *response = data;
  return sizeof(data);
}

static int set_report(struct GrainuumUSB *usb,
                          const void *packet,
                          const void **response)
{
  asm("bkpt #43");
                          ...
                          const void **response)
{
  ...
  // If it's a SET_REPORT or GET_REPORT, return that.
  if ((setup->bmRequestType & 0xf0) == 0xa0) {
    if (setup->bRequest == GET_REPORT)
      return get_report(usb, setup, response);
    else if (setup->bRequest == SET_REPORT)
      return set_report(usb, setup, response);
    else
      return 0;
  }
  if ((setup->bmRequestType == 0x21) && (setup->bRequest == SET_IDLE))
    return 0;
//  if ((setup->bmRequestType & 0xf0) != 0x80)
//    asm("bkpt #45");
  ...
  case DT_HID_REPORT:
    return get_set_hid_report_descriptor(usb, setup, response);
  case 0:
    return 0;
  default:
    asm("bkpt #45");
